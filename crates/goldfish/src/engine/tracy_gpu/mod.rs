...
impl TracyVkContext {
	pub fn new(
		...
		vk_get_physical_device_calibrateable_time_domains: Option<
		...
		unsafe {
			match (
				...
				(
					Some(vk_get_physical_device_calibrateable_time_domains),
					Some(vk_get_calibrated_timestamps),
				) => {
					let mut num: u32 = 0;
					vk_get_physical_device_calibrateable_time_domains(
						physical_dev,
					...
					vk_get_physical_device_calibrateable_time_domains(
						physical_dev,
						&mut num as *mut u32,
						&mut data as *mut vk::TimeDomainEXT,
					)
					.result()
					.unwrap();
